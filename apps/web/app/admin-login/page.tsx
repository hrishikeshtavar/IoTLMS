...
const SimuSoftLogo = ({ height = 32 }: { height?: number }) => (
  <svg xmlns="http://www.w3.org/2000/svg" xmlSpace="preserve" height={height}
    ...
    <defs>
      <style>{`.al0{fill:#2B2A29;fill-rule:nonzero}.al1{fill:#0078BF;fill-rule:nonzero}.al2{fill:url(#alg)}.al3{fill:#FEFEFE;fill-rule:nonzero}.ast0{stroke:#FEFEFE;stroke-width:26.46;stroke-linecap:square;stroke-linejoin:round;stroke-miterlimit:5.01585;fill:none;fill-rule:nonzero}`}</style>
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F4F9FF', fontFamily: "'Nunito', sans-serif", display: 'flex', flexDirection: 'column' }}>
      ...
      <style>{`
        ...
        .al-input:focus { border-color:#0078BF !important; box-shadow:0 0 0 4px rgba(0,120,191,0.1) !important; outline:none; }
        .al-btn:hover:not(:disabled) { transform:translateY(-2px); box-shadow:0 10px 28px rgba(0,120,191,0.35) !important; }
      ...
      <nav style={{ background: 'rgba(255,255,255,0.95)', backdropFilter: 'blur(12px)', borderBottom: '1px solid #E2EDF5', padding: '0 2.5rem', height: 64, display: 'flex', alignItems: 'center', justifyContent: 'space-between', boxShadow: '0 1px 12px rgba(0,120,191,0.06)' }}>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <Link href="/login" style={{ padding: '7px 16px', borderRadius: 9, color: '#4A7A96', fontWeight: 700, fontSize: '0.83rem', textDecoration: 'none' }}>Student Login</Link>
          <Link href="/" style={{ padding: '7px 14px', borderRadius: 8, color: '#4A7A96', fontWeight: 700, fontSize: '0.83rem', textDecoration: 'none' }}>← Back to Home</Link>
        </div>
      </nav>
      ...
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem', position: 'relative', overflow: 'hidden' }}>
        ...
        <svg style={{ position: 'absolute', top: '8%', right: '6%', opacity: 0.09, animation: 'float 5s ease-in-out infinite alternate', pointerEvents: 'none' }} width="90" height="90" viewBox="0 0 90 90" fill="none">
          <rect x="22" y="22" width="46" height="46" rx="6" stroke="#0078BF" strokeWidth="2.5" fill="rgba(0,120,191,0.06)"/>
        ...
        <div style={{ width: '100%', maxWidth: 420, animation: 'fadeUp 0.5s ease both' }}>
          ...
          <div style={{ background: '#fff', borderRadius: '0 0 20px 20px', padding: '36px 40px 32px', boxShadow: '0 8px 40px rgba(0,120,191,0.1)', border: '1px solid #E2EDF5', borderTop: 'none' }}>
            ...
            <div style={{ textAlign: 'center', marginBottom: 28 }}>
              <div style={{ width: 56, height: 56, borderRadius: 16, background: 'linear-gradient(135deg,#EBF5FF,#E0F5FA)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.8rem', margin: '0 auto 12px', border: '1px solid #B8D9F5' }}>🏫</div>
              <div style={{ fontFamily: "'Plus Jakarta Sans',sans-serif", fontWeight: 800, fontSize: '1.2rem', color: '#0D1A2A', lineHeight: 1.2 }}>Admin Portal</div>
              <div style={{ fontSize: '0.8rem', color: '#7BA8BE', fontWeight: 600, marginTop: 4 }}>SimuLearning · SimuSoft Technologies</div>
            </div>

            {/* Role tabs */}
            <div style={{ display: 'flex', background: '#F0F7FF', borderRadius: 12, padding: 4, marginBottom: 24, border: '1px solid #D0E8F5' }}>
              {([{ k: 'admin', icon: '🏫', label: 'School Admin' }, { k: 'super', icon: '⚙️', label: 'Super Admin' }] as {k:'admin'|'super',icon:string,label:string}[]).map(r => (
                <div key={r.k} onClick={() => setRole(r.k)}
                  style={{ flex: 1, padding: '9px 8px', borderRadius: 9, background: role === r.k ? '#0078BF' : 'transparent', cursor: 'pointer', textAlign: 'center', transition: 'all 0.2s', userSelect: 'none' }}>
                  <div style={{ fontSize: '0.8rem', fontWeight: 800, color: role === r.k ? '#fff' : '#4A7A96' }}>{r.icon} {r.label}</div>
                </div>
              ))}
            </div>

            {/* Role hint */}
            <div style={{ background: '#F0F7FF', borderRadius: 10, padding: '10px 14px', marginBottom: 20, border: '1px solid #D0E8F5', fontSize: '0.78rem', color: '#4A7A96', lineHeight: 1.5 }}>
              {role === 'admin'
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div>
                <label style={{ fontSize: '0.82rem', fontWeight: 700, color: '#2A4A5E', display: 'block', marginBottom: 6 }}>
              ...
              <div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                  ...
                  <Link href="/forgot-password" style={{ fontSize: '0.78rem', color: '#0078BF', fontWeight: 700, textDecoration: 'none' }}>Forgot?</Link>
                </div>
            ...
            <div style={{ marginTop: 20, textAlign: 'center' }}>
              <span style={{ fontSize: '0.8rem', color: '#7BA8BE' }}>Not an admin? </span>
              <Link href="/login" style={{ fontSize: '0.8rem', color: '#0078BF', fontWeight: 700, textDecoration: 'none' }}>Student Login →</Link>
            </div>
          </div>

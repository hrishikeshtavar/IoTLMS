  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)' }}>
      ...
      <section style={{ background: 'linear-gradient(135deg, #0D1B2E 0%, #0F172A 50%, #0D2137 100%)', padding: 'clamp(4rem,8vw,7rem) 2rem', position: 'relative', overflow: 'hidden' }}>
        ...
        {/* glow orbs */}
        <div style={{ position: 'absolute', top: '-10%', right: '10%', width: 400, height: 400, background: 'radial-gradient(circle, rgba(26,115,232,0.15) 0%, transparent 70%)', pointerEvents: 'none' }} />
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', position: 'relative', zIndex: 1, display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '3rem', alignItems: 'center' }}>
          <div>
            <div className="animate-fadeUp" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'rgba(26,115,232,0.15)', border: '1px solid rgba(26,115,232,0.3)', borderRadius: 'var(--radius-full)', padding: '6px 16px', marginBottom: '1.5rem' }}>
              <span style={{ width: 6, height: 6, borderRadius: '50%', background: '#1A73E8', display: 'inline-block' }} />
              <span style={{ fontSize: '0.82rem', fontWeight: 600, color: '#93C5FD', fontFamily: 'DM Sans' }}>{t.tagline}</span>
            </div>
            ...
            <h1 className={`animate-fadeUp delay-200 ${isDevanagari ? 'lang-hi' : ''}`} style={{ fontFamily: 'Plus Jakarta Sans, sans-serif', fontWeight: 900, fontSize: 'clamp(2.2rem,5vw,3.5rem)', background: 'linear-gradient(135deg, #60A5FA, #34D399)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text', lineHeight: 1.1, letterSpacing: '-0.03em', marginBottom: '1.5rem' }}>
              {t.hero2}
            </h1>

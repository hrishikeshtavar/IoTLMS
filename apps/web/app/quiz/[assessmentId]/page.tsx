...
async function cacheAssessment(id: string, data: Assessment) {
  ...
  tx.objectStore('assessments').put({ id, data, cachedAt: Date.now() });
}
...
async function cacheSubmission(submission: object) {
  ...
  tx.objectStore('pending_submissions').put({ ...submission, id: Date.now().toString() });
}
...
export default function QuizPage() {
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:3001/api/assessments/${assessmentIdParam}`)
      ...
      .then(async (data: Assessment) => {
        setAssessment(data);
        await cacheAssessment(assessmentIdParam, data);
        setLoading(false);
      })
      .catch(async () => {
        // Offline fallback
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center text-gray-500">Loading quiz...</div>;
  if (!assessment) return <div className="min-h-screen flex items-center justify-center text-gray-500">Quiz not found</div>;
  ...
  return (
    <main className="min-h-screen bg-gray-50">
      <div className="bg-white border-b px-6 py-4 flex items-center justify-between">
        <button onClick={() => router.back()} className="text-blue-600 text-sm hover:underline">← Back</button>
        <h1 className="text-lg font-bold text-blue-600">IoTLearn Quiz</h1>
        <div className="flex items-center gap-3">
          {isOffline && (
            <span className="px-2 py-1 bg-orange-100 text-orange-600 rounded text-xs font-medium">📡 Offline Mode</span>
          )}
          ...
          <div className="bg-white rounded-xl border p-8 text-center">
            {isOffline ? (
              <>
                ...
                <h2 className="text-2xl font-bold mb-2">Saved for Sync</h2>
                <p className="text-gray-500 mb-6">You're offline. Your answers have been saved and will sync when you reconnect.</p>
              </>
            ) : (
              <>
                <div className="text-6xl mb-4">{result.passed ? '🎉' : '😔'}</div>
                <h2 className="text-2xl font-bold mb-2">{result.passed ? 'Quiz Passed!' : 'Quiz Failed'}</h2>
                <p className="text-gray-500 mb-6">You scored {result.score}/{result.max_score} ({result.percentage}%)</p>
                ...
                <div className="text-left space-y-3 mb-8">
                  {result.results.map((r, i) => (
                    <div key={i} className={`p-3 rounded-lg flex items-center gap-3 ${r.is_correct ? 'bg-green-50' : 'bg-red-50'}`}>
                      <span>{r.is_correct ? '✅' : '❌'}</span>
                      <span className="text-sm text-gray-700">
                        Q{i + 1}: {r.is_correct ? 'Correct' : `Wrong — answer was "${r.correct_answer}"`}
                      </span>
                      <span className="ml-auto text-sm font-medium">+{r.points_earned} pts</span>
                    </div>
                  ))}
                </div>
              </>
          ...
          <>
            ...
            {question && (
              <div className="bg-white rounded-xl border p-6 mb-6">
                <h2 className="text-lg font-semibold text-gray-800 mb-6">{question.question_text}</h2>
                <div className="space-y-3">
                  {(question.options_json ?? []).map(opt => (
                    <button key={opt.value} onClick={() => setAnswers(prev => ({ ...prev, [question.id]: opt.value }))}
                      ...
                      <span className="font-medium mr-3">{opt.value}.</span>{opt.label}
                    </button>
                  ))}
                </div>
              </div>

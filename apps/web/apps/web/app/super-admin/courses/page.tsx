...
function CourseForm({ course, tenants, onSave, onClose }: { course: Partial<Course>|null; tenants: Tenant[]; onSave:(c:Course)=>void; onClose:()=>void }) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (res.ok) { const data = await res.json(); setMsg(isEdit ? 'Course updated!' : 'Course created!'); setTimeout(() => onSave(data), 700); }
  ...
  return (
    <div style={{ position:'fixed', inset:0, zIndex:200, background:'rgba(0,0,0,0.55)', display:'flex', justifyContent:'flex-end' }}
      ...
      <div style={{ background:'#fff', width:'min(640px,100vw)', height:'100vh', overflowY:'auto', boxShadow:'-8px 0 40px rgba(0,0,0,0.15)', display:'flex', flexDirection:'column' }}>
        <div style={{ background:'linear-gradient(135deg,#1A73E8,#00C896)', padding:'1.5rem', flexShrink:0 }}>
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
            <div>
              <div style={{ color:'rgba(255,255,255,0.75)', fontSize:'0.8rem', fontWeight:600, marginBottom:4 }}>{isEdit ? 'EDITING COURSE' : 'NEW COURSE'}</div>
        ...
        <div style={{ padding:'1.5rem', flex:1, display:'flex', flexDirection:'column', gap:'1.25rem' }}>
          ...
          <div>
            ...
            <div style={{ display:'flex', borderBottom:'1px solid #e5e7eb', marginBottom:'0.75rem' }}>
              {(['en','hi','mr'] as const).map(l => (
                <button key={l} onClick={() => setTab(l)}
                  style={{ flex:1, padding:'0.5rem', border:'none', cursor:'pointer', fontWeight:700, fontSize:'0.8rem', background:tab===l?'#EFF6FF':'#fff', color:tab===l?'#1A73E8':'#6b7280', borderBottom:tab===l?'2px solid #1A73E8':'2px solid transparent' }}>
            ...
            {tab==='en' && (
              <div style={{ display:'flex', flexDirection:'column', gap:'0.75rem' }}>
                ...
                <textarea value={form.description_en} onChange={e => set('description_en', e.target.value)} placeholder="Course description in English..." rows={6} style={{ ...inputStyle, resize:'vertical', lineHeight:1.6 }} />
              </div>
            )}
            {tab==='hi' && (
              <div style={{ display:'flex', flexDirection:'column', gap:'0.75rem' }}>
                <input value={form.title_hi} onChange={e => set('title_hi', e.target.value)} placeholder="हिंदी में कोर्स का शीर्षक" style={{ ...inputStyle, fontFamily:'Noto Sans Devanagari' }} />
                <textarea value={form.description_hi} onChange={e => set('description_hi', e.target.value)} placeholder="हिंदी में कोर्स का विवरण..." rows={6} style={{ ...inputStyle, resize:'vertical', fontFamily:'Noto Sans Devanagari', lineHeight:1.6 }} />
              </div>
            )}
            {tab==='mr' && (
              <div style={{ display:'flex', flexDirection:'column', gap:'0.75rem' }}>
                <input value={form.title_mr} onChange={e => set('title_mr', e.target.value)} placeholder="मराठीत कोर्सचे शीर्षक" style={{ ...inputStyle, fontFamily:'Noto Sans Devanagari' }} />
                <textarea value={form.description_mr} onChange={e => set('description_mr', e.target.value)} placeholder="मराठीत कोर्सचे वर्णन..." rows={6} style={{ ...inputStyle, resize:'vertical', fontFamily:'Noto Sans Devanagari', lineHeight:1.6 }} />
              </div>
            )}
          </div>
          ...
          <div>
            ...
            <div style={{ fontSize:'0.72rem', color:'#9ca3af', marginTop:4 }}>Auto-generated from title. Used in course URLs.</div>
          </div>
            ...
            <div style={{ display:'flex', gap:'0.75rem' }}>
              {(['draft','published'] as const).map(s => (
                <button key={s} onClick={() => set('status', s)}
                  style={{ flex:1, padding:'0.6rem', borderRadius:8, border:'1.5px solid', borderColor:form.status===s?'#1A73E8':'#e5e7eb', background:form.status===s?'#EFF6FF':'#fff', color:form.status===s?'#1A73E8':'#6b7280', fontWeight:700, fontSize:'0.85rem', cursor:'pointer' }}>
          ...
          <button onClick={handleSave} disabled={saving}
            style={{ padding:'0.875rem', borderRadius:12, background:saving?'#9ca3af':'linear-gradient(135deg,#1A73E8,#00C896)', color:'#fff', border:'none', fontWeight:800, fontSize:'1rem', cursor:saving?'not-allowed':'pointer', marginTop:'auto' }}>
    ...
    <div style={{ minHeight:'100vh', background:'#f9fafb', fontFamily:'system-ui,sans-serif' }}>
      <nav style={{ position:'sticky', top:0, zIndex:50, background:'#fff', borderBottom:'1px solid #e5e7eb', padding:'0 1.5rem', height:60, display:'flex', alignItems:'center', justifyContent:'space-between', boxShadow:'0 1px 6px rgba(0,0,0,0.06)' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:'0.75rem' }}>
          ...
          <button onClick={() => setCreating(true)} style={{ padding:'8px 18px', borderRadius:8, background:'linear-gradient(135deg,#1A73E8,#00C896)', color:'#fff', border:'none', fontWeight:700, fontSize:'0.875rem', cursor:'pointer' }}>+ New Course</button>
        ...
        <div style={{ display:'flex', gap:'0.75rem', marginBottom:'1.5rem', flexWrap:'wrap', alignItems:'center' }}>
          <div style={{ position:'relative', flex:1, minWidth:200 }}>
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search courses..."
              style={{ width:'100%', padding:'0.65rem 1rem 0.65rem 2.5rem', borderRadius:999, border:'1.5px solid #e5e7eb', fontSize:'0.9rem', fontFamily:'inherit', outline:'none', boxSizing:'border-box' }} />
          </div>
          <select value={tenantFilter} onChange={e => setTenantFilter(e.target.value)}
            style={{ padding:'0.65rem 1rem', borderRadius:999, border:'1.5px solid #e5e7eb', fontSize:'0.875rem', background:'#fff', cursor:'pointer' }}>
          ...
          <div style={{ display:'flex', gap:'0.4rem' }}>
            {(['all','published','draft'] as const).map(f => (
              <button key={f} onClick={() => setStatusFilter(f)}
                style={{ padding:'0.5rem 1rem', borderRadius:999, border:'1.5px solid', borderColor:statusFilter===f?'#1A73E8':'#e5e7eb', background:statusFilter===f?'#1A73E8':'#fff', color:statusFilter===f?'#fff':'#6b7280', fontWeight:700, fontSize:'0.8rem', cursor:'pointer', textTransform:'capitalize' }}>
                {f}
              </button>
            ))}
          </div>
        ...
        <div style={{ background:'#fff', borderRadius:16, border:'1px solid #e5e7eb', overflow:'hidden', boxShadow:'0 2px 8px rgba(0,0,0,0.04)' }}>
          ...
          {loading ? (
            ...
            <div style={{ textAlign:'center', padding:'4rem', color:'#9ca3af' }}>
              ...
              No courses found.{' '}
              <button onClick={() => setCreating(true)} style={{ color:'#1A73E8', fontWeight:700, background:'none', border:'none', cursor:'pointer' }}>Create one →</button>
            </div>
          ) : filtered.map(course => {
              ...
              <div key={course.id}
                ...
                <div>
                  <button onClick={() => toggleStatus(course)}
                    style={{ fontSize:'0.72rem', padding:'3px 10px', borderRadius:999, fontWeight:700, cursor:'pointer', border:'none', background:course.status==='published'?'#DCFCE7':'#F3F4F6', color:course.status==='published'?'#15803D':'#6b7280' }}>
                    {course.status==='published'?'🟢 Live':'📝 Draft'}
                  </button>
                </div>
                <div style={{ display:'flex', gap:'0.4rem', justifyContent:'flex-end' }}>
                  <button onClick={() => setEditing(course)} style={{ padding:'4px 12px', borderRadius:6, border:'1.5px solid #e5e7eb', background:'#fff', color:'#1A73E8', fontSize:'0.75rem', fontWeight:700, cursor:'pointer' }}>Edit</button>
                  <button onClick={() => deleteCourse(course.id)} disabled={deleting===course.id}
                    style={{ padding:'4px 10px', borderRadius:6, border:'1.5px solid #fecaca', background:'#FFF5F5', color:'#DC2626', fontSize:'0.75rem', fontWeight:700, cursor:'pointer', opacity:deleting===course.id?0.5:1 }}>
      ...
      {(creating || editing) && (
        <CourseForm course={editing} tenants={tenants} onSave={handleSaved} onClose={() => { setCreating(false); setEditing(null); }} />
      )}
    </div>
  );
}
